...
def get_args():
    ...
    #--X train_expression.csv --Y phen_train.csv --testX test_expression.csv --testY phen_test.csv
    parser.add_argument('--input', type=str, required=False, help='Input DAG file')
    parser.add_argument('--query', type=str, required=False, help='Query in the form: start end | [list of observed variables]')
...
def main():
    args=get_args()
    if args.input:
        lines=np.genfromtxt(args.X, skip_header=1)
    else:
